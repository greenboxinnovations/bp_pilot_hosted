...
if(isset($_GET['t'])){
	...
	$sql = "SELECT a.*,b.car_no_plate, c.cust_company,c.cust_disp_name,c.cust_ph_no,d.pump_name 
			FROM `transactions` a
			JOIN `cars` b
	...
	if(mysqli_num_rows($exe) > 0){
		while($row = mysqli_fetch_assoc($exe)) {
			...
			//--------------------------------//
			// customer details
			$display_name = $row['cust_disp_name'];
			...
			echo '<div id="cust_details">';
				...
				echo '<div id="cust_display_name">'.ucwords($pump_name).'</div>';
				...
				$today      = date('Y-m-d');
				$check_date = date('Y-m-d',(strtotime ( '-7 day' , strtotime ( $today) ) ));

				$timestamp1 = strtotime($date_dir); 
  				$timestamp2 = strtotime($check_date); 
				...
				echo '<div class="title inline">'.ucwords($display_name).'</div>'; 

				echo '<div class="title inline">'.strtoupper($row['car_no_plate']).'</div>'; 
				// if ($btn) {
				...
				echo'<div class="container" >';
					echo '<div class="title inline">T-NO</div>';  echo '<div class="val inline">'.($row['transaction_no']).'</div>';
				echo '</div>';
				...
				echo'<div class="container" >';
					echo '<div class="title inline">Product</div>';  echo '<div class="val inline">'.ucwords($row['fuel']).'</div>';
				echo '</div>';

				echo'<div class="container" >';
					echo '<div class="title inline">Amount(Rs)</div>';  echo '<div class="val inline">'.$row['amount'].'</div>';
				echo '</div>';
			...
			if ($url_switch) {
				$vid_path = 'https://fuelcambucket.s3.ap-south-1.amazonaws.com/videos/'.$date_dir."/".$pump_id."/".$trans_string.".mp4";
				 
			}else{
				$vid_path = $video_dir."/".$date_dir."/".$pump_id."/".$trans_string.".mp4";
			}
			...
			// if(file_exists($vid_path)) {
				...
				echo '<div class="img_desc">FUELLING VIDEO (10X SPEED)</div>';
				echo '<video controls>
			...
			foreach ($check as $i => $extention) {
				...
				if ($url_switch) {
					$file_path = 'https://fuelcambucket.s3.ap-south-1.amazonaws.com/uploads/'.$date_dir."/".$pump_id."/".$trans_string.$extention;
				}else{
					$file_path = $upload_dir."/".$date_dir."/".$pump_id."/".$trans_string.$extention;
				}
				...
				// if(file_exists($file_path)) {
					...
					echo '<img src="'.$file_path.'" onerror="this.src=css/nf_small.jpg">';
					echo '</div>';
					echo '</div>';
					// echo '<br>';
				// }else{
